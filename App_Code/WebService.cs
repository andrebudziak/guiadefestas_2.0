...
public class WebService : System.Web.Services.WebService 
{
    ...
    public int IncluirBanner(int codigo, int codigo_cliente, string descricao, int posicaoBanner,int largura,int altura,int status)
    {
    ...
    public DataSet ConsultaBanner(int codigo_banner)
    {
        ...
        if (codigo_banner == 0)
        {
            comando = "Select b.codigo,b.descricao,b.codigo_local_banner,b.largura,b.altura, "+
                      "b.status, l.descricao as local,b.codigo_cliente "+
                      "from [banner] b "+
    ...
    public DataSet ConsultaAnuncioTela(int codigo_categoria, string nome_fantasia,string ordem)
    {
        ...
        string comando = "";
        if ((codigo_categoria != 0) && (ordem == "0"))
        {
            comando = " SELECT a.codigo,c.nome_fantasia,a.cep,a.bairro,a.cidade,a.endereco,l.orkut,l.facebook,l.twitter, "+
                      "a.telefone,a.email,a.site,a.status,lo.descricao AS logo,b.codigo as bonus  "+
                      "FROM anuncio a  "+
                      "INNER JOIN cliente c ON a.codigo_cliente = c.codigo "+
                      "INNER JOIN anuncio_categoria ac ON ac.codigo_anunciante=a.codigo_cliente  "+
                      "INNER JOIN logo lo ON lo.codigo_anuncio = a.codigo_cliente  "+
                      "LEFT JOIN link l on c.codigo = l.codigo_anuncio   "+
                      "LEFT JOIN bonus b ON a.codigo_cliente = b.codigo_anunciante and b.status=1  "+
                      "where ac.codigo_categoria="+codigo_categoria.ToString()+" and a.status=1 " +
                      "ORDER BY NEWID()  ";
        }

        if ((nome_fantasia != "0"))
        {
            comando = " SELECT DISTINCT a.codigo,c.nome_fantasia,a.cep,a.bairro,a.cidade,a.endereco,l.orkut,l.facebook,l.twitter, " +
                      "a.telefone,a.email,a.site,a.status,lo.descricao AS logo,b.codigo as bonus  " +
                      "FROM anuncio a  " +
                      "INNER JOIN cliente c ON a.codigo_cliente = c.codigo " +
                      "INNER JOIN anuncio_categoria ac ON ac.codigo_anunciante=a.codigo_cliente  " +
                      ...
                      "LEFT JOIN link l on c.codigo = l.codigo_anuncio   " +
                      "LEFT JOIN bonus b ON a.codigo_cliente = b.codigo_anunciante and b.status=1  " +
                      " WHERE c.nome_fantasia like '%" + nome_fantasia + "%' and a.status=1  ";
        }
        ...
        if ((ordem != "0"))
        {
            comando = " SELECT a.codigo,c.nome_fantasia,a.cep,a.bairro,a.cidade,a.endereco,l.orkut,l.facebook,l.twitter, " +
                      ...
                      "LEFT JOIN link l on a.codigo = l.codigo_anuncio   " +
                      "LEFT JOIN bonus b ON a.codigo_cliente = b.codigo_anunciante and b.status=1  " +
                      "WHERE ac.codigo_categoria = " + codigo_categoria.ToString() + " and a.status=1  ORDER BY " + ordem + "  ";
        }
    ...
    [WebMethod(Description = "Dados TAG Clientes")]
    public DataSet DadosTagClientes()
    {

        SqlConnection MyConnection = new SqlConnection();
        MyConnection.ConnectionString = connstring;

        SqlCommand cmd = new SqlCommand(" DECLARE @Names VARCHAR(8000) "+
                                        "SELECT @Names = '' "+
                                        "SELECT @Names = @Names + ',' + nome_fantasia FROM cliente " +
                                        "Select @Names as nomes ", MyConnection);
        SqlDataAdapter adpt = new SqlDataAdapter(cmd);

        // Cria e preenche o DataTable
        DataTable dtBanner = new DataTable("tag");
        adpt.Fill(dtBanner);
        DataSet ds = new DataSet();
        ds.Tables.Add(dtBanner);
        cmd.Dispose();
